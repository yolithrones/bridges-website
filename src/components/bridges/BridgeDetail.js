...
const BridgeDetail = () => {
  ...
  return (
    <DetailContainer>
      ...
      <HeroSection>
        <HeroImage src={bridge.mainImage} alt={bridge.name} />
        <HeroOverlay>
          <Title
          ...
          >
            <FaMapMarkerAlt />
            {bridge.location}, {bridge.country}
          </Location>
        </HeroOverlay>
      </HeroSection>
      ...
      <ContentGrid>
        <MainContent>
          ...
          <InfoList>
            <InfoItem>
              <FaCalendarAlt />
              Completed: {bridge.completionYear}
            </InfoItem>
            <InfoItem>
              <FaRuler />
          ...
          {bridge.gallery && bridge.gallery.length > 0 && (
            <>
              ...
              <ImageGallery>
                {bridge.gallery.map((image, index) => (
                  <GalleryImage
                    key={index}
                    src={image}
                    alt={`${bridge.name} - Image ${index + 1}`}
        ...
        <Sidebar>
          ...
          >
            ...
            <InfoList>
              <InfoItem>Category: {bridge.category}</InfoItem>
              <InfoItem>Continent: {bridge.continent}</InfoItem>
              <InfoItem>Status: {bridge.status}</InfoItem>
              {bridge.architect && (
                <InfoItem>Architect: {bridge.architect}</InfoItem>
              )}
            </InfoList>
          </InfoCard>
          ...
          >
            ...
            <Description>{bridge.historicalSignificance}</Description>
          </InfoCard>
        </Sidebar>
      </ContentGrid>
    </DetailContainer>
